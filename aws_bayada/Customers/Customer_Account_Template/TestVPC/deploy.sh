...
if [ "$#" -lt 5 ]
then
  echo "Usage: ./deploy {terraform_function} {vpcsubnet} {env} {profile} {account} from directory of tf code"
  exit 1
fi
...
terraform $FUNCTION -var "vpcsubnet=$VPCSUBNET" -var "environment=$ENVIRONMENT" -var "profile=$PROFILE"
