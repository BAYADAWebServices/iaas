...
if [ "$#" -lt 5 ]
then
  echo "Usage: ./deploy {terraform_function} {env} {profile} {account} {custgwattachid} from directory of tf code"
  exit 1
fi
...
terraform $FUNCTION -var "environment=$ENVIRONMENT" -var "profile=$PROFILE" -var "cust_tgw_attach_id=$CUSTGWATTACHID"
